...
class App extends Component {
  ...
  async componentDidMount() {
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      let link = `https://meta-weather.vercel.app/api/location/search/?lattlong=${lat},${lng}`
      fetch(link)
        ...
        .then((result) => {
          ...
          let locationLink = `https://meta-weather.vercel.app/api/location/${this.state.myLocation.woeid}/`
          fetch(locationLink)
            .then((response) => response.json())
            .then((result) => {
              this.setState({
